#!/usr/bin/env python3
"""
Convergence Grid Summary - Analyze N and L convergence studies for P2
"""
import argparse
import glob
import json
...
def main():
    ...
    ap.add_argument("--out_csv", default="figs/convergence_grid.csv")
    args = ap.parse_args()

    results = []
    
    if args.grid_type == "N":
        # N convergence: look for sweeps/conv_grid_N*/summary.csv
        pattern = "sweeps/conv_grid_N*/summary.csv"
        param_re = re.compile(r"conv_grid_N(\d+)")
        param_name = "N"
    else:
    ...
    print(f"[conv-summary] Found {len(files)} {args.grid_type} convergence files")
    ...
    if not results:
        print(f"Error: No valid {args.grid_type} convergence results found")
        return
    ...
    if len(slopes) > 1:
        ...
        print(f"\n[conv-analysis] {param_name} convergence:")
