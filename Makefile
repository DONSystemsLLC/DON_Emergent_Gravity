PYTHON ?= python
PY ?= $(PYTHON)
P1_ORBIT_SCRIPTS := experiments/P1_Kepler_Orbits/scripts
P1_KERNEL_SCRIPTS := experiments/P1_Emergent_Kernel/scripts
P2_WL_SCRIPTS := experiments/P2_Weak_Lensing/scripts

.PHONY: wl-run wl-monitor wl-finalize wl-check wl-plot wl-gate orb-run orb-finalize orb-smoke orb-smoke-hot orb-parse

wl-run:
	$(PY) -u $(P2_WL_SCRIPTS)/clens_random_ensemble.py --patch_dir working/CLENS_patch --out_prefix working/CLENS_random_strat3 --n_ensembles 200 --seed 42 --stack_with $(P2_WL_SCRIPTS)/clens_stack.py --zbuf 0.2 --rmin 0.2 --rmax 2.0 --nbin 12 --resume

wl-monitor:
	while true; do date; $(PY) -u $(P2_WL_SCRIPTS)/monitor_clens.py; sleep 20; done

wl-finalize:
	$(PY) -u $(P2_WL_SCRIPTS)/clens_finalize_results.py
	$(PY) -u $(P2_WL_SCRIPTS)/clens_plots.py
	$(PY) -u $(P2_WL_SCRIPTS)/clens_slip_analysis.py
	$(PY) -u $(P2_WL_SCRIPTS)/validate_sign.py
	$(PY) -u $(P2_WL_SCRIPTS)/accept_slip_gate.py
...
wl-check:
	$(PY) -u $(P2_WL_SCRIPTS)/monitor_clens.py
...
orb-run:
	...
	$(PY) -u $(P1_ORBIT_SCRIPTS)/orbit_metrics_sweep.py \
...
orb-parse:
	$(PY) -u $(P1_ORBIT_SCRIPTS)/parse_orbit_logs.py sweeps/N320_L160_vtheta_sweep && \
	cat sweeps/N320_L160_vtheta_sweep/summary.csv

orb-finalize:
	$(PY) -u $(P1_ORBIT_SCRIPTS)/orbit_finalize_results.py --summary sweeps/N320_L160_vtheta_sweep/summary.csv
...
proof-p1:
	...
	$(PY) $(P1_KERNEL_SCRIPTS)/analyze_field_slope_v2.py
	@echo ">> Building rotation curve from slope profile ..."
	$(PY) $(P1_KERNEL_SCRIPTS)/plot_rotation_curve.py
...
conv-grid:
	...
	$(PY) -u $(P2_WL_SCRIPTS)/convergence_grid_summary.py --grid_type N --out_csv figs/convergence_grid.csv
...
conv-box:
	...
	$(PY) -u $(P2_WL_SCRIPTS)/convergence_box_summary.py --grid_type L --out_csv figs/convergence_box.csv
...
kepler-runs:
	...
	$(PY) -u $(P1_ORBIT_SCRIPTS)/parse_orbit_logs.py sweeps/kepler_v*
...
ep-runs:
	...
	$(PY) -u $(P1_ORBIT_SCRIPTS)/parse_orbit_logs.py sweeps/ep_m*
...
# -------------------
gate-strict:
	$(PY) -u $(P1_KERNEL_SCRIPTS)/acceptance_gate.py \
...
appendix-md:
	$(PY) -u $(P1_KERNEL_SCRIPTS)/appendix_methods_table_md.py \
...
appendix-tex:
	$(PY) -u $(P1_KERNEL_SCRIPTS)/appendix_methods_table_tex.py \
	  --methods_csv figs/methods_table.csv \
...
paper-kit: conv-grid conv-box slope-window helmholtz kepler-fit ep-fit wl-finalize
	...
	$(PY) -u $(P1_KERNEL_SCRIPTS)/build_report_p2.py \
	  ...
	  --methods_csv figs/methods_table.csv \
	  --out docs/REPORT.md
	$(PY) -u $(P2_WL_SCRIPTS)/build_acceptance_box_p2.py \
